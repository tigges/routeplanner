...
page_cfg = dict(title=cfg['title'], slug=cfg['slug'], by=cfg.get('by', ''), pickerDefault=cfg.get('pickerDefault', 'all'), routeWord=cfg.get('routeWord', 'national routes'), landFill=bool(cfg.get('graph', {}).get('land_fill')), trips=TRIPS, networks=NETWORKS, lang=cfg['lang'], proj=json.loads(grab('CFG'))['proj'] if re.search(r'^var CFG=', src, re.M) else None,
