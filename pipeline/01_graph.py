...
FERRY = {(f['from'], f['to'], f.get('variant', 'base')): f for f in cfg.get('ferries', [])}   # a ferry (boat, car train) listed
used = set()                                                                                 # between two consecutive trunk towns replaces the ride leg
for t in trunks:
    v = t['variant']
